#include "26445549_mq.h"

// Function that sends messages of type 1 which contains the unix commands to the message queue
void sendMessageType1FromInput(char *line, FILE *fp, struct msgbuf *mbuf, int msqid) {
  // Keep iterating until there is no more line to be read from the input file (fp->file pointer)
  while (fgets(line, MSQ_SIZE, fp)) {
    strcpy(mbuf->mtext, removeTrailingNewlineChar(line)); // Copy each line in input file to the message buffer text
    mbuf->mtype = 1; // Set message of type 1 (unix commands)

    // Attempt to send the message to the message queue
    ...
    if (msgsnd(msqid, mbuf, MSQ_SIZE, 0) < 0) {
...
// Function that sends messages of type 2 which indicates the end of the unix commands to the message queue
void sendMessageType2(struct msgbuf *mbuf, int msqid) {
  mbuf->mtype = 2; // Set message of type 2 (end of the unix commands)
  memset(mbuf->mtext, 0, MSQ_SIZE); // Initialise the message buffer text
...
// Function that removes trailing newline character from each line in message buffer text ("line" variable) as a result of the fgets()
...
// Function that gets command from message queue (assign the command and its argument(s) as element(s) of the command array)
void getCommandFromLine(char *line, char *command[BUFFER_SIZE], int *argumentCounter) {
  ...
  char tmpLine[BUFFER_SIZE]; // Temporary variable that also holds the message buffer text ("line" variable)
  char *token; // Represent each substring of the message buffer text ("line" variable) delimited/separated by " "

  // Copy the content of the message buffer text ("line" variable) to the temporary variable ("tmpLine" variable)
  ...
  // Get a substring of the message buffer text delimited/separated by " "
  // Keep iterating until there is no more substring to be retrieved from the message buffer text
...
// Function that executes command from message queue
